...
export function SectionPagination() {
  ...
  return (
    <>
      ...
      {/* Boutons de navigation section par section - DROITE */}
      <motion.div
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.5 }}
        className="fixed right-8 top-1/2 z-50 hidden -translate-y-1/2 flex-col gap-3 xl:flex"
      >
      ...
      {/* Boutons empilés en bas à droite */}
        ...
        <AnimatePresence>
          {showTopButton && (
            <motion.button
              ...
              onClick={scrollToTop}
              className="flex h-16 w-16 items-center justify-center overflow-hidden bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-2xl transition-all hover:scale-110 tech-corner"
              whileHover={{ y: -4 }}
              whileTap={{ scale: 0.9 }}
        ...
        <AnimatePresence>
          {scrollProgress < 95 && (
            <motion.button
              ...
              className="group flex h-16 w-16 items-center justify-center overflow-hidden bg-white shadow-2xl transition-all hover:bg-primary-500 hover:scale-110 tech-corner dark:bg-gray-800"
              whileHover={{ y: 4 }}
              whileTap={{ scale: 0.9 }}
              title="Aller en bas"
            >
